...
export async function GET(
  ...
  try {
    const res = await fetch(`${process.env.LARAVEL_API_URL}/api/sensors/${id}`, {
      headers: laravelHeaders(token),
    });
    const data = await res.json();
    return NextResponse.json(data, { status: res.status });
  } catch (err) {
    console.error(`[GET /api/sensors/${id}]`, err);
...
export async function PUT(
  ...
  try {
    ...
    console.error(`[PUT /api/sensors/${id}]`, err);
    return NextResponse.json({ message: 'Server error.' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error(`[DELETE /api/sensors/${id}]`, err);
    return NextResponse.json({ message: 'Server error.' }, { status: 500 });
  }
}
