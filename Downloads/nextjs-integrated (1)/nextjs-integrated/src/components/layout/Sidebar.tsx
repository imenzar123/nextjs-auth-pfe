...
import { ADMIN_ONLY_PATHS } from '@/backend/lib/constants';
...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  // Hide admin-only items for non-admin users (middleware blocks the routes
  // server-side too — this is purely cosmetic / UX).
  const visibleNavItems = navItems.filter((item) => {
    const isAdminOnly = (ADMIN_ONLY_PATHS as readonly string[]).includes(item.href);
